...
function setupSimulatorPage() {
    ...
    shovelBtn.addEventListener('click', () => {
        if (state === 'mature') {
            ...
            shovelOverlay.className = 'notification-overlay';
            shovelOverlay.style.position = 'fixed';
            shovelOverlay.style.zIndex = '100';

            const box = document.createElement('div');
            box.className = 'notification-box';
            box.style.animation = 'pop 0.4s ease-out forwards';
            
            const treeClone = treeContainer.cloneNode(true);
            treeClone.id = 'dialog-tree';
            
            treeClone.style.display = 'block';
            treeClone.style.position = 'relative';
            treeClone.style.transform = 'scale(0.6)';
            treeClone.style.transformOrigin = 'bottom center';
            treeClone.style.bottom = 'auto';
            treeClone.style.left = 'auto';
            treeClone.style.margin = '0 auto';
            
            box.innerHTML = `
                <h3>Here is your tree!</h3>
                <div id="dialog-tree-container" style="width: 100%; height: 240px; display: flex; justify-content: center; align-items: flex-end; position: relative;"></div>
                <p class="quote" style="font-style: italic; color: #666; max-width: 80%; margin: 15px auto 0 auto; text-align: center;">"${randomQuote}"</p>
                <button id="confirm-shovel">View Calendar</button>
            `;
            
            box.querySelector('#dialog-tree-container').appendChild(treeClone);
            shovelOverlay.appendChild(box);
            document.body.appendChild(shovelOverlay);
            
            shovelOverlay.style.display = 'flex';
